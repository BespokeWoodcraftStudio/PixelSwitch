...
final class MCPServer {
    ...
    static let tools: [Tool] = {
        ...
        return [
            ...
            Tool(name: "list_accounts", description: "Every account in priority order, with its usage, its switch threshold (manualOnly true: auto-switch never switches to it; effectiveThreshold is where auto-switch moves off it while it is active), and whether it can be switched to.",
            ...
            Tool(name: "switch_account", description: "Make this account the one Claude Code uses on that Mac. Returns when the switch is done. Works for a manual-only account too; auto-switch then moves off it only at the default threshold.",
            ...
            Tool(name: "set_account_threshold", description: "Set the usage percentage (1–100) at which auto-switch moves off this account; another account is switched to only while it is at least 10 points under its own threshold (at or under half of it, below 20). 0 makes it manual only: auto-switch never switches to it; a person or switch_account still can, and while it is active auto-switch moves off it at the default threshold. Null or omitted: follow the default threshold. 100 uses the account until it is empty.",
                 properties: account.merging(["threshold": .object(["type": .array([.string("number"), .string("null")]), "minimum": .number(0), "maximum": .number(100)])]) { a, _ in a },
