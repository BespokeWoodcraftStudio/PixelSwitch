...
struct LiteLLMModelPricing: Sendable {
    ...
    func cost(input: Int, output: Int, cacheCreate: Int, cacheCreate1h: Int, cacheRead: Int, isFast: Bool) -> Double {
        ...
        // every model PixelSwitch ingests (bare ids from ~/.claude/projects). The
...
enum PricingSource: Sendable {
    ...
    case fresh(fetchedAt: Date)         // ~/Library/Application Support/PixelSwitch/litellm-pricing-fresh.json
...
actor PricingService {
    ...
    private static let freshURL: URL = {
        ...
        let dir = appSupport.appendingPathComponent("PixelSwitch", isDirectory: true)
