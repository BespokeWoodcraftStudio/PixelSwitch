...
/// Drop-in replacement for os.Logger that writes to ~/Library/Logs/PixelSwitch-app.log.
/// Rotates at 5MB, keeps 2 prior files (.1 and .2) for ~15MB total cap.
/// Usage: `let log = FileLog("Category"); log.info("message")`
/// Read logs: `cat ~/Library/Logs/PixelSwitch-app.log` or `tail -f ~/Library/Logs/PixelSwitch-app.log`
...
private final class FileLogWriter: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.pixelventures.pixelswitch.filelog")
    ...
    init() {
        ...
        self.basePath = logsDir + "/PixelSwitch-app.log"
        ...
        try? FileManager.default.removeItem(atPath: logsDir + "/PixelSwitch.log")
    ...
    private func writeSessionHeader() {
        ...
        let header = "====== PixelSwitch launched \(ts) ======\n"
